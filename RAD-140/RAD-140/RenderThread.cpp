    ...
    if (esp) {
        ...
        for (int i = 0; i < 64; i++) {
            ...
            if (localTeam != team) {
                ...
                ImGui::GetForegroundDrawList()->AddRect(ImVec2(screen.x - 3.0f, screen.y - 3.0f), ImVec2(screen.x + 3.0f, screen.y + 3.0f), IM_COL32(255, 0, 0, 255), 1, 1.0f);
            }
        }
    }
...
void RenderOverlayThread() {
	...
	while (shouldRenderMenu) {
        ...
        std::thread espThread(RenderESPThread);
        espThread.join();
		overlay.EndRender();
